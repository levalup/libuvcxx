...
namespace uvcxx {
    ...
    public:
        ...
        void emit(void *p) const UVCXX_NOEXCEPT {
            try {
                ...
                (void) raise(std::current_exception());
            }
        }
         ...
         */
        bool raise(const std::exception_ptr &p) const UVCXX_NOEXCEPT {
            if (m_on_watch) {
                try {
    ...
    public:
        ...
        self &call(std::function<void(T...)> f) {
            m_core->call([UVCXX_CAPTURE_MOVE(f)](void *p) mutable {
                auto &pack = *(type *) (p);
        ...
        self &call(FUNC f) {
            return this->call(on_call_t([UVCXX_CAPTURE_MOVE(f)](T ...args) mutable {
        ...
        template<typename FUNC, typename std::enable_if<
                sizeof...(T) != 0 && std::is_same<
                        ...
                        void>::value, int>::type = 0>
        self &call(FUNC f) {
            return this->call(on_call_t([UVCXX_CAPTURE_MOVE(f)](T...) mutable {
                f();
            }));
        }
    ...
    public:
         ...
         */
        virtual bool raise(const std::exception_ptr &p) const UVCXX_NOEXCEPT = 0;
        ...
        bool raise(ARGS &&...args) const UVCXX_NOEXCEPT {
            return this->raise(std::make_exception_ptr(E(std::forward<ARGS>(args)...)));
        }

        bool raise() const UVCXX_NOEXCEPT {
            return this->raise(std::current_exception());
        }
    ...
    public:
        ...
        bool raise(const std::exception_ptr &p) const UVCXX_NOEXCEPT final {
            return m_core->raise(p);
        }
    ...
    public:
        ...
        void emit(T ...v) const UVCXX_NOEXCEPT final {
            try {
                ...
                (void) self::raise(std::current_exception());
                return;
            }
        }
